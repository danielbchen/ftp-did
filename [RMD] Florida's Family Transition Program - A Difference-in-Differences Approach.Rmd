---
title: "Florida's Family Tranistion Program - A Difference-in-Differences Approach"
author: "Daniel Chen"
date: "June 2021"
output:
...
```{r Test Employment Rates Between Groups, message=FALSE}
# Load libraries
library(tidyverse)
library(foreign)
library(broom)
...
```{r Regress Employment Status on TLyes}
ftp %>%
  filter(post_treat == 0) %>%
...
```{r Plot Pre-Treatment Estimates, warning=FALSE}
pre_treat_results %>%
  mutate(
    period = case_when(
...
```{r Diff-in-Diff Clustered Standard Errors}
# Run DiD using plm() function and store output to object
clustered_plm <- plm(
...
```{r Visualizing Heterogenous Treatment Effects, warning=FALSE}
...
post_treat_results %>%
  mutate(
    period = as.numeric(str_extract(string = term, pattern = '\\d+')) + 9
  ) %>%
  ggplot(aes(x = period, y = estimate)) +
  geom_point() + 
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high)) + 
